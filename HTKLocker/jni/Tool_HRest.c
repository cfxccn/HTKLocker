...
void PrintInitialInfoHRest(void)
{
...
void CheckDataHRest(char *fn, BufferInfo info)
{
	char tpk[80];
	char mpk[80];
...
void InitSegStoreHRest(BufferInfo *info)
{
	Observation obs;
	Boolean eSep;
...
void LoadFileHRest(char *fn)
{
	...
	CheckDataHRest(fn, info);
	if (firstTime) InitSegStoreHRest(&info);
